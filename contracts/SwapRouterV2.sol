...
contract SwapRouterV2 {
    ...
    bytes constant MAIN_PAIR_INIT_CODE =
        hex'533e9fb2a4807fcffe2086e95f38f687cb0e9ea0371ab06446ce4970993bebb2';
    ...
    receive() external payable {
        assert(msg.sender == WAVAX); // only accept ETH via fallback from the WAVAX contract
    }
    ...
    function _addLiquidity(
        ...
        if (
            IUniswapV2Factory(factoryMain).getPair(tokenA, tokenB) == address(0)
        ) {
            IUniswapV2Factory(factoryMain).createPair(tokenA, tokenB);
        }
        (uint reserveA, uint reserveB) = SwapRouterV2Library.getReserves(
            factoryMain,
            tokenA,
            tokenB,
            MAIN_PAIR_INIT_CODE
        ...
        if (reserveA == 0 && reserveB == 0) {
            ...
            uint amountBOptimal = SwapRouterV2Library.quote(
                amountADesired,
                reserveA,
                reserveB
            );
            if (amountBOptimal <= amountBDesired) {
                ...
                uint amountAOptimal = SwapRouterV2Library.quote(
                    amountBDesired,
                    reserveB,
                    reserveA
                );
        ...
        returns (uint amountToken, uint amountETH, uint liquidity)
    {
        ...
        IWAVAX(WAVAX).deposit{value: amountETH}();
        assert(IWAVAX(WAVAX).transfer(pair, amountETH));
        liquidity = IUniswapV2Pair(pair).mint(to);
    ...
    function swapExactETHForTokens(
        ...
        require(path[0] == WAVAX, 'UniswapV2Router: INVALID_PATH');
        amounts = SwapRouterV2Library.getAmountsOut(
            isUni ? factorySecondary : factoryMain,
            msg.value,
            isUni ? 0 : IUniswapV2Factory(factoryMain).swapFeeBP(),
    ...
    function swapTokensForExactETH(
        ...
        uint256 withdrawAmount = isUni
            ...
            : amounts[amounts.length - 1] -
                ...
                10000;
        IWAVAX(WAVAX).withdraw(withdrawAmount);
