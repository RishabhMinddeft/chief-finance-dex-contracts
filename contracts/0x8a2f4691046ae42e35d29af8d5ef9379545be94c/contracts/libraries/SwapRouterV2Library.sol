...
library SwapRouterV2Library {
    ...
    function getAmountOut(uint amountIn, uint reserveIn, uint reserveOut, uint fee) internal pure returns (uint amountOut) {
        ...
        amountOut = numerator / denominator - (numerator / denominator) * fee / 10000;
    }
    ...
    function getAmountsOut(address factory, uint amountIn, uint fee, address[] memory path, bytes memory initCode) internal view returns (uint[] memory amounts) {
    ...
    function getAmountsIn(address factory, uint amountOut, uint fee, address[] memory path, bytes memory initCode) internal view returns (uint[] memory amounts) {
        require(path.length >= 2, 'UniswapV2Library: INVALID_PATH');
        amounts = new uint[](path.length);
